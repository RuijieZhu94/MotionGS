...
class InputPadder:
    """ Pads images so that dimensions are divisible by the padding factor """

    def __init__(self, dims, mode='sintel', padding_factor=16):
        self.wd, self.ht = dims[-2:]
        pad_ht = (((self.ht // padding_factor) + 1) * padding_factor - self.ht) % padding_factor
        pad_wd = (((self.wd // padding_factor) + 1) * padding_factor - self.wd) % padding_factor
        if mode == 'sintel':
            self._pad = [pad_ht // 2, pad_ht - pad_ht // 2, pad_wd // 2, pad_wd - pad_wd // 2]
        else:
...
class GMFlow(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        # results_dict = {}
        flow_preds = []
        padder = InputPadder(img1.shape, padding_factor=16)
        img0, img1 = padder.pad(img0.cuda(), img1.cuda())
...
def build_gmflow(cfg):
    model = GMFlow(num_scales=cfg.num_scales,

...
def ArrayToTorch(array, resolution):
    resized_image_torch = torch.from_numpy(array).float() # h x w x (c)
    assert resized_image_torch.shape[:2] == resolution[::-1], f"Array shape {resized_image_torch.shape} does not match resolution {resolution}"

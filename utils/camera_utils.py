...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if args.resolution in [1, 2, 4, 8]:
        resolution = round(orig_w / (resolution_scale * args.resolution) / 16) * 16, round(
            orig_h / (resolution_scale * args.resolution) / 16) * 16 # round to nearest multiple of 16
    ...
    if cam_info.depth is not None:
        depth = ArrayToTorch(cam_info.depth, resolution)
    else:
        depth = None

    if cam_info.motion_mask is not None:
        motion_mask = ArrayToTorch(cam_info.motion_mask, resolution)
    else:
        motion_mask = None
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  ...
                  depth=depth, motion_mask=motion_mask, resolution=resolution, meta_only=args.dataloader)
